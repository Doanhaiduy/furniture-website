...
-- ── public_products ─────────────────────────────────────────────────────────
-- Only the product_translations join changes (hard locale -> lateral fallback).
-- Also fixes a latent filter bug: p_price_max was accepted but never applied.
...
BEGIN
  ...
  SELECT
    ...
    p.price_unit::text
  FROM products p
  -- BUG 2 FIX: prefer requested locale, fall back to vi, then any translation.
  JOIN LATERAL (
    SELECT t.*
  ...
  WHERE
    ...
    -- S3: a product is public only while its category is PUBLISHED. Archiving or
    -- unpublishing a category hides all of its products from list/search/filter,
    -- matching how blog already gates on its category status.
    ...
    -- BUG (latent) FIX: apply the max-price filter that was previously ignored.
