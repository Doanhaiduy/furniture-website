...
const newProductCatsLoad = `        const catsList = Array.isArray(cats) ? cats : (cats as any)?.data || [];
        const concreteCats = catsList.filter((c: any) => c.parent_id !== null);
        const formattedCats = concreteCats.map((c: any) => {
